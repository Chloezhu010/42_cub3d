...
int load_all_textures(t_game *game)
{
    ...
    if (!load_texture(game, &game->textures.north_img, game->textures.north_path))
        return (0);
    if (!load_texture(game, &game->textures.south_img, game->textures.south_path))
        return (0);
...
t_img *get_wall_texture(t_game *game, t_wall_side side)
{
    switch(side) {
        ...
        default:
            return &game->textures.north_img; // 默认值
    }
}